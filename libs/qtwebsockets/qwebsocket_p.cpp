    ...
    if (Q_LIKELY(!m_pSocket)) {
        ...
        if (url.scheme() == QStringLiteral("wss")) {
            if (!QSslSocket::supportsSsl()) {
                const QString message = QWebSocket::tr("SSL Sockets are not supported on this platform.");
            ...
            const QString message = QWebSocket::tr("Unsupported WebSocket scheme: %1").arg(url.scheme());
            setErrorString(message);
            emitErrorOccurred(QAbstractSocket::UnsupportedSocketOperationError);
        }
    }
...
qint64 QWebSocketPrivate::doWriteFrames(const QByteArray& data, bool isBinary)
{
    ...
    Q_Q(QWebSocket);
    const QWebSocketProtocol::OpCode firstOpCode = isBinary ? QWebSocketProtocol::OpCodeBinary : QWebSocketProtocol::OpCodeText;
...
void QWebSocketPrivate::processHandshake(QTcpSocket* pSocket)
{
    ...
    case ParsingHeaderState: {
        ...
        if (Q_LIKELY(m_httpStatusCode == 101)) {
            ...
            ok = !(acceptKey.isEmpty() || (m_httpMajorVersion < 1 || m_httpMinorVersion < 1) || (upgrade.toLower() != QStringLiteral("websocket")) || (connection.toLower() != QStringLiteral("upgrade")));
            if (ok) {
                const QString accept = calculateAcceptKey(m_key);
                ok = (accept == acceptKey);
                if (!ok)
                    errorDescription = QWebSocket::tr("Accept-Key received from server %1 does not match the client key %2.")
                                           .arg(acceptKey)
                                           .arg(accept);
            } else {
                errorDescription = QWebSocket::tr("QWebSocketPrivate::processHandshake: Invalid statusline in response: %1.")
            ...
            if (!version.isEmpty()) {
                ...
                if (!versions.contains(QString::number(QWebSocketProtocol::currentVersion()))) {
                    ...
                    errorDescription = QWebSocket::tr("Handshake: Server requests a version that we don't support: %1.")
                                           .arg(versions.join(QStringLiteral(", ")));
                } else {
                    // we tried v13, but something different went wrong
                    errorDescription = QWebSocket::tr("QWebSocketPrivate::processHandshake: Unknown error condition encountered. Aborting connection.");
                }
            } else {
                errorDescription = QWebSocket::tr("QWebSocketPrivate::processHandshake: Unknown error condition encountered. Aborting connection.");
            }
        } else {
            errorDescription = QWebSocket::tr("QWebSocketPrivate::processHandshake: Unhandled http status code: %1 (%2).")
                                   .arg(m_httpStatusCode)
                                   .arg(m_httpStatusMessage);
        }
        if (ok)
    ...
    QList<QPair<QString, QString>> headers)
{
    ...
    handshakeRequest << QStringLiteral("GET ") % resourceName % QStringLiteral(" HTTP/1.1") << QStringLiteral("Host: ") % host << QStringLiteral("Upgrade: websocket") << QStringLiteral("Connection: Upgrade") << QStringLiteral("Sec-WebSocket-Key: ") % QString::fromLatin1(key);
    if (!origin.isEmpty())
...
{
    return (m_pSocket && m_pSocket->isValid() && (m_socketState == QAbstractSocket::ConnectedState));
}
